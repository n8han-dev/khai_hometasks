...
int main(void)
{
    ...
    {
        printf("incorrect ticket number, should not be grater than 54\ninput ticket number: ");
        scanf("%d", &n);
    }
    ...
    if (n<36)
    {
        ...
        /*
        if coupe place we divide ticket number by 4 because 4 seats for 1 coupe and round it to bigger number, 
    ...
    {
        c = (int)round((n-36)/2.0-0.1); // compartment id if side palce
        printf("c: %d\n", c);
        /*
        if coupe place we retract 36 from ticket number thendivide ticket number by 2 because 4 seats for 1 coupe and round it to bigger number, 
        then retract 1 because id starts from 0 and tickets from 1
        */
        side = "side";
        comp = comps[8-c];
